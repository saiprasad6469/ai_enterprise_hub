...
async function connectDB() {
    ...
    if (!env_1.env.MONGODB_URI || env_1.env.MONGODB_URI.includes('<db_password>')) {
    ...
    const finalUri = sanitizeMongoUri(env_1.env.MONGODB_URI);
    try {
        const conn = await mongoose_1.default.connect(finalUri, {
